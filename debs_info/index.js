...
$(document).ready(function () {
    ...
    if (!dPackage) {
        $(".package-error").text("Có vẻ như bạn đang cố truy cập thông tin từ kho lưu trữ của tôi :))").css("display", "block");
    ...
    $.getJSON("packages/" + dPackage + ".json", function (data) {
        ...
        if (typeof currentVersion === 'undefined' &&
            ...
            if (supported) {
                ...
                $(".panel-body.version-check").css("background-color", "#19E500").css("border-radius", "12px");
            } else{
                result += "Phiên bản ios hiện tại <strong>Chưa hỗ trợ</strong>";
                result += (typeof currentVersion != 'undefined') ? " (" + currentVersion + ")" : "";
                result += " &#128531;";
                $(".panel-body.version-check").css("background-color", "#ffcc00").css("border-radius", "12px");
            }
            $(".version-check").html(result);
        }
